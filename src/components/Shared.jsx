...
export function WhatsApp() {
  useEffect(() => {
    ...
    s.id = 'faris-wa-css'
    s.textContent = `
      @keyframes wa-dot { 0%,100%{opacity:1} 50%{opacity:.3} }
      .wa-pill { transition: box-shadow .25s ease, transform .25s ease; }
      .wa-pill:hover { transform: translateY(-3px) !important;
        box-shadow: 0 12px 32px rgba(37,211,102,.45) !important; }
    `
  ...
  return (
    <a href={`https://wa.me/212642499661?text=${encodeURIComponent(msg)}`}
      target="_blank" rel="noopener noreferrer" className="wa-pill"
      style={{
        position: 'fixed', bottom: 24, right: 16, zIndex: 150,
        display: 'flex', alignItems: 'center', gap: 10,
        background: 'white', borderRadius: 50,
        padding: '10px 18px 10px 12px',
        boxShadow: '0 4px 20px rgba(0,0,0,.13), 0 1px 4px rgba(0,0,0,.08)',
        textDecoration: 'none', border: '1px solid rgba(0,0,0,.06)'
      }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 1 }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#25d366',
            display: 'inline-block', animation: 'wa-dot 1.5s ease infinite' }} />
          <span style={{ fontSize: 10, color: '#25d366', fontWeight: 700,
            fontFamily: 'Inter,sans-serif', letterSpacing: .5 }}>متاح الآن</span>
        </div>
